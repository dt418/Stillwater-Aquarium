...
window.habitatRate = (fps) => {
  requestedRate = Number.isFinite(fps) && fps > 0 ? Math.min(360, fps) : 0;
...
function fail(error) {
  ...
  loading.textContent = "Không khởi tạo được WebGL2. Bật tăng tốc phần cứng và thử mở lại. " + error.message;
  loading.style.cssText = "padding:10vh 10vw;color:#d7e5dd;pointer-events:auto";
}
...
async function start() {
  ...
  renderer.toneMappingExposure = prefs.light / 100 * 1.17;
  ...
  canvas.addEventListener("pointermove", (event) => {
    if (!prefs.follow) { pointer = null; return; }
    const bounds = canvas.getBoundingClientRect();
  ...
  document.addEventListener("keydown", (event) => {
    if (event.repeat || /INPUT|SELECT|BUTTON/.test(event.target.tagName)) return;
    if (event.code === "Space") {
      event.preventDefault();
      window.stillwaterSet("paused", !paused);
    }
  ...
  loop = createFrameLoop(renderFrame, {
    fps: requestedRate, paused: prefs.paused || paused, hidden: document.hidden || zeroSize || contextLost,
  });
